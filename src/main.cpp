...
uint256 hashGenesisBlock("0xcc5c60e63037920621eea4981c1e06b81ca2432ed2026b46a27256920d6e6311");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Eijecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Eijecoin Signed Message:\n";
...
{
    // Eijecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3 * 60 * 60; // Eijecoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Eijecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Eijecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Eijecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Eijecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcc5c60e63037920621eea4981c1e06b81ca2432ed2026b46a27256920d6e6311");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Eije me, eije you, I think eiji needs to do a ...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x70dbfc7a254797b37c7a32f9cffc07b84d1932835dedcf61c467b7fc6a8c6a8a"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xf0, 0xf6, 0xcb }; // Eijecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EijecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EijecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EijecoinMiner(CWallet *pwallet)
{
    printf("EijecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eijecoin-miner");
    ...
    try { loop {
        ...
        printf("Running EijecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EijecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EijecoinMiner, pwallet));
}
